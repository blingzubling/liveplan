...
.controller('View3Ctrl', ['$scope', '$http', '$routeParams', 'gabiObject', function($scope, $http, $routeParams, gabiObject) {
    ...
    var paintProcessInst = function(piJson) {
        ...
        var pi = s.rect(
            ...
            piJson["height"] - strokeWidth).attr({
            stroke: "#e1e1e1",
    ...
    var paintFlowInst = function(fiJson) {
        ...
        if ($scope.sorted.length > 0) {
            ...
            s.polyline(pfad).attr({
                fill: "none",
                stroke: decimalToRGB(fiJson["color"]),
                "stroke-width": "3"
            });
            ...
            if (arrowDirection.isLeftToRight()) {} else if (arrowDirection.isRightToLeft()) {
